...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setAppName("WikipediaRanking").setMaster(s"local[*]")
  ...
  def main(args: Array[String]) {
    ...
    /* Languages ranked according to (1) */ //naive ranking took 19889 ms.
    val langsRanked: List[(String, Int)] = timed("Part 1: naive ranking", rankLangs(langs, wikiRdd))

    /* An inverted index mapping languages to wikipedia pages on which they appear */
    def index: RDD[(String, Iterable[WikipediaArticle])] = makeIndex(langs, wikiRdd)

    /* Languages ranked according to (2), using the inverted index */ //inverted index took 24684 ms.
    val langsRanked2: List[(String, Int)] = timed("Part 2: ranking using inverted index", rankLangsUsingIndex(index))

    /* Languages ranked according to (3) */ //reduceByKey took 17304 ms.
